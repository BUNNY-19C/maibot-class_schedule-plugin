...
- **截图里的公式自动认出来**:图片笔记除存原图外,还会送进视觉模型识别成
  LaTeX + 中文名称 + 知识点,落进公式库并打标签。**同一张图只识别一次**
  (图片 hash 缓存,不重复计费),**同一公式只存一条**(LaTeX 归一化后取指纹去重),
  没把握的标 `#待确认` 而不是编个名字。识别在后台队列做,不阻塞聊天;
  没配 API Key 就自动跳过,笔记照收。
