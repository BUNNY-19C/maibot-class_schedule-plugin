...
- **截图里的公式自动认出来**:图片笔记除存原图外,还会送进视觉模型识别成
  LaTeX + 中文名称 + 知识点,落进公式库并打标签;**一页课件里有几条公式就收几条**。
  **全自动,不需要你为识别做任何操作**——发图时自动认,Key 是后来才配的、或某次识别
  网络失败过,插件会在装载时把库里还没有公式的图片自动补识别一遍。**同一张图只识别
  一次**(图片 hash 缓存,不重复计费),**同一公式只存一条**(LaTeX 归一化后取指纹
  去重),没把握的标 `#待确认` 而不是编个名字。公式回执**不经过任何模型、只发原文**
  ——让宿主模型复述既会把公式概括掉,它又没看到那张图、会凭空描述内容跟主链路抢话。
  识别在后台队列做,不阻塞聊天;没配 Key 就自动跳过,笔记照收。
...
| `/归到 课程名` | 把最近一条「未分类」笔记归到指定课程；支持简称（自动控制 → 航空自动控制基础）与错字，多候选时回复编号选择 |
| `/找 关键词` | 跨课程搜索学习笔记 |
| `/笔记库` | 笔记库状态:条数/状态分布/公式数,以及公式识别的累计结果与队列积压 |
| `/归到 1～N` | 从上一条 /归到 列出的候选里选一个（只作用于发起时的那条笔记） |
| `/重识图片 [课程名]` | 清掉图片的识别缓存并按当前模型重新识别(升级模型/提示词后用) |
...
tool_query_enabled = false    # LLM 查课表(默认关:名单和私聊限制都管不到它)
...
- **LLM 工具不受访问名单与适用范围限制,所以默认关闭**。麦麦调用工具时不传
  会话信息(已实测),"只走私聊"管不住这条路;需要这个能力再打开
  `access.tool_query_enabled`。
