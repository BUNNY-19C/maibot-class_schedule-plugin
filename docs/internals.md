...
| 公式识别(VLM) | `pipeline` 的 worker 队列(`_run_inbox_item`) | 只少一条公式:重试一次降级模型后放弃,原因进日志与 `/笔记库` |
| 历史图片补识别 | 装载与识别刚可用时的后台任务(`_run_formula_backfill`) | 单张失败不影响其它图;同一张图的自动尝试有上限 |
...
675 项单元测试,只用标准库 `unittest`,不需要额外安装依赖:
