...
不是重新识别)、图里没有公式、失败,四种都说。曾经只在"新公式"时才说话,结果同一批
课件图重发时一片安静,用户以为功能坏了(线上实测)。异步链路的每一步也都留日志
(`识别完成`/`命中图片缓存`/`图里没有公式`/`失败`),否则"用户觉得没反应"时无从下手。

**数据不能被拟人化掉,而模型又看不到图**:`_deliver(verbatim=True)` 直接发
`fixed_text`,**一次模型都不问**。两条都是线上实测的教训:① persona 下宿主模型把
公式概括成"公式没错,和之前一样",LaTeX 根本没送到用户手上;② 改成"拟人一句+原文
附后"之后又踩到新坑——插件现编那句拟人话时模型**没看到那张图**,凭空描述图片内容
(真实出现过"那个矩形波例题喵…上课别摸鱼"),与主链路对同一条消息的回复撞车,
用户看到的是重复发言。

**一张图可以有多条公式**:提示词要求"有几条写几条"(上限 `MAX_FORMULAS_PER_IMAGE`)、
逐字照抄、看不清的局部用 `?` 占位并压低 confidence、只有确实没公式才返回空数组。
识别器返回 ``{"status", "formulas": [...], "degraded", "error"}``,回执一条消息列全;
每条各自算指纹、各自打标、各自去重。**``{"formulas": []}`` 是合法结果(图里真没
公式),不算失败、不吃自动重试预算**;而 ``{"latex": ""}`` 这种"我放弃了"的形状按
解析失败处理,让降级模型再试一次。
