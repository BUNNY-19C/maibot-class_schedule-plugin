...
662 项单元测试,只用标准库 `unittest`,不需要额外安装依赖:
...
- **识别结果不保证正确**:视觉模型会认错符号、认错名称。低置信(名称「未知公式」
  或置信度 < 0.6)会打 `#待确认`,但**高置信也可能是错的**——公式是要拿去算题的,
  请以原图为准,`#待确认` 只是提示而非保证。
- **识别是异步的,可能丢**:队列满(`study.queue_size`)会丢弃识别任务并计数,
  插件卸载时队列里未做完的识别也会消失。丢的只是识别,笔记与回执不受影响,
  下次同样内容重新入队(图片 hash 缓存与指纹去重保证不会重复存)。
- **没识别成功的图片不会自动重试**:失败只记一次日志与 `/笔记库` 里的计数,
  不会后台反复重试(避免配错地址时无限烧时间)。想重试就重发一张图
  (注意同一张图会命中图片缓存,除非换图或清掉对应公式记录)。
- **公式库当前只能通过识别与检索使用**:没有删除/改正公式的命令,
  需要人工修正时直接改 `notes.db`(原始层 `latex_raw` 只增不改,归一化与标签可重算)。
