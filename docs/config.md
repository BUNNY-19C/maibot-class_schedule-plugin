...
| `auto_subscribe` | `true` | 导入成功后自动把当前会话设为提醒对象(不用再发 /课表订阅) |
...
| 字段 | 默认 | 说明 |
| --- | --- | --- |
| `enabled` | `true` | 开启按课收纳公式/重点/图片 |
| `capture_keywords` | 见下 | 触发词列表:「记一下」「这个是公式」「这个是重点」等,可增删 |
| `arm_minutes` | `10` | 只发触发词时,等待内容的时长 |
